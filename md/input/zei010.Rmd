...
t1 <- datatable(cbind(country=met[-1],sapply(data[-1,],as.numeric)),
                rownames=F, colnames=c("",
                                       as.character(data.y[1,]),
                                       format(as.yearqtr(as.character(data.q[1,]),"%Y-%m"),
                                              ifelse(params$lang=="FR","T%q","Q%q"))
                ),
                options = list(paging=F,searching=F,info=F,
                               language=list(decimal=","),
                               columnDefs = list(list(className = 'dt-right', defaultContent="nd",targets = 1:16))
                )) %>%
