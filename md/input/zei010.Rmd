...
t1 <- htmlwidgets::prependContent(
  datatable(cbind(country=met[-1],sapply(data[-1,],as.numeric)),
            ...
            options = list(paging=F,searching=F,info=F,
                           ...
                           rowCallback=DT::JS(
                             highlightTableRowByCountry(country="ea",begin=0,end=16)
                           )
            ),
        class="compact") %>%
    ...
    formatStyle(1,target="row",
                ...
                # paste0(c(style.color.FR,style.color.EA),';'))
                # c('1px solid blue','1px solid red'))
    ),
  tags$style(sprintf(
    '#%s table.dataTable {
