...
meta.currencies <- unique(eta.currencies[rownames(data.y99),]$TITLE)
...
for (i in 1:length(meta.currencies))
  ...
  #View(rbind(t(as.data.frame(lapply(meta.currencies,function(x){c(x,rep(" ",ncol(data)-1))}),stringsAsFactors=F)),data))
data <- rbind(
  as.data.frame(matrix(rep(NA,ncol(data)*length(meta.currencies)),ncol=ncol(data),nrow=length(meta.currencies)),stringsAsFactors=F),
  data)
