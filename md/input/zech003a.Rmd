...
series.col <- "OBS_VALUE"
data.y99 <- data.y99.0[with(data.y99.0, order(match(data.y99.0$CURRENCY,series), EXR_SUFFIX)),]
data.y99 <- data.y99[,c(series.col)]
data.y <- data.y.0[with(data.y.0, order(match(data.y.0$CURRENCY,lapply(series,`[[`,1)), EXR_SUFFIX)),]
data.y <- data.y[,c("date",series.col)]
#head(data.y.0[with(data.y.0, order(match(CURRENCY,lapply(series,`[[`,1)),EXR_SUFFIX,date)),])

#set series title
meta <- rep(ifelse(params$lang=="FR",c("Fin de période","Moyenne sur la période"),c("End of period","Period average")),length(data.y99)/2)

# data.y99 <- data.frame()
# data.y99 <- lapply(series,function(ts){
#   with(data.y99.0,
#        eval(parse(text=series.col))[eval(parse(text=names(ts[1])))==ts[[1]] &
#                                       eval(parse(text=names(ts[2])))==ts[[2]]])
# })
